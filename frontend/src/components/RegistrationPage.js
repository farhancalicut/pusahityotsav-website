...
function RegistrationPage() {
  ...
  const validateStep = (step) => {
    if (step === 0) {
      const { fullName, email, state, gender, groupId, course, phoneNumber } =
        formData;
      if (
        !fullName ||
        !email ||
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedEventIDs = Object.keys(formData.events).filter(
      (id) => formData.events[id],
    );
    ...
    try {
      const contestantResponse = await axios.post(
        `${API_BASE_URL}/api/contestants/`,
        contestantPayload,
      );
      ...
      if (details?.email) {
        userMessage = `Email Error: ${details.email[0]}`;
      }
  ...
  const getEligibleEvents = () => {
    ...
    return allEvents.filter(
      (event) => event.category === selectedCategory.name,
    );
  };
          ...
          <>
            ...
            >
              ...
              <Box sx={{ p: 2, backgroundColor: "#f5f5f5" }}>
                <Typography>
                  A person can participate upto four individual Programmes.
                  Additionally there is no limits for participating in general
                  items.
                </Typography>
              </Box>
            </Paper>
            ...
            <FormGroup>
              {eligibleEvents.length > 0 ? (
                ...
                <Typography>
                  Please select your category to see eligible events.
                </Typography>
              )}
            </FormGroup>
          </>
